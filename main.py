...
def apresentar_metricas_finais(historico_testes, tipo_amb):
    """Gera o relatório baseado nas métricas específicas do PDF"""
    
    num_testes = len(historico_testes)
    if num_testes == 0: return

    # Dados acumulados
    total_recompensa = sum(h['recompensa'] for h in historico_testes)
    total_sucessos = sum(1 for h in historico_testes if h['sucesso'])
    ...
    # 1. MÉTRICAS AMBIENTE FAROL [cite: 152-155]
    if tipo_amb == '1':
        media_passos = total_passos / num_testes
        ...
        print(f"   • Passos Médios:       {media_passos:.1f} (Menor é melhor)")

    # 2. MÉTRICAS AMBIENTE LABIRINTO [cite: 160-163]
    elif tipo_amb == '2':
        media_visitadas = sum(h['celulas_visitadas'] for h in historico_testes) / num_testes
        print(f"-> CENÁRIO: LABIRINTO")
        print(f"   • Taxa de Sucesso:     {(total_sucessos/num_testes)*100:.1f}%")
        print(f"   • Passos até Saída:    {total_passos/num_testes:.1f}")
        print(f"   • Células Visitadas:   {media_visitadas:.1f} (Exploração)")

    # 3. MÉTRICAS AMBIENTE RECOLEÇÃO [cite: 156-159]
    elif tipo_amb == '3':
        total_recursos = sum(h['recursos_recolhidos'] for h in historico_testes)
        total_colisoes = sum(h['colisoes'] for h in historico_testes)
        
        # Eficiência = Recursos / Passos
        eficiencia = total_recursos / total_passos if total_passos > 0 else 0
        
        print(f"-> CENÁRIO: RECOLEÇÃO")
        print(f"   • Recursos Recolhidos: {total_recursos} (Total nos testes)")
        print(f"   • Média por Episódio:  {total_recursos/num_testes:.1f}")
        print(f"   • Colisões Média:      {total_colisoes/num_testes:.1f}")
        print(f"   • Eficiência:          {eficiencia:.4f} recursos/passo")
...
def main():
    ...
    print(f"\n[MODO TESTE] Executando {num_testes} episódios...")
    
    for i in range(num_testes):
        motor.executa_episodio(max_passos=60, visual=True, delay=0.1)
        
        # Recolher dados do episódio
        sucesso_ep = getattr(env, 'objetivo_alcancado', False)
        recursos = getattr(env, 'pontuacao_total', 0)
        
        # Lógica de sucesso específica
        if tipo_amb == '3' and recursos > 0: sucesso_ep = True
        
        # Guarda estatísticas detalhadas deste teste
        dados_teste = {
            'sucesso': sucesso_ep,
            'recompensa': agente.recompensa_acumulada,
            'passos': env.passo_atual,
            'celulas_visitadas': len(agente.stats['celulas_visitadas']),
            'colisoes': agente.stats['colisoes'],
            'recursos_recolhidos': recursos
        }
        historico_testes.append(dados_teste)
        
        status = "Sucesso" if sucesso_ep else "Falha"
        print(f"   Episódio {i+1}: {status} (R: {agente.recompensa_acumulada:.1f})")
        time.sleep(0.5)

    # APRESENTA AS MÉTRICAS DO RELATÓRIO
